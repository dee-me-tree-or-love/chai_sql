...
spec = do
    ...
    THS.describe "Parser" $ do
        ...
        THS.describe "on typed SQL syntax" $ do
            THS.describe "parses" $ do
                let i = "--@cs :: {name: Text}\n\
                    \SELECT * FROM Cats;"
                THS.it ("`" ++ i ++ "` into a correct AST") $ do
                    let t = Just [TAST.TAstSimpleAtomicIndexKeyValue (TAST.TAstSimpleIndexKey "name") TAST.TAstAtomicTypeText]
                    let a = [AST.AstSelectAttributeAccessStar AST.AstSelectAttributeStarTotalRecord]
                    let f = [AST.AstFromAccessReference (AST.AstVariable "Cats")]
                    let e = AST.AstSelectQuery t a f
                    THS.shouldBe (p i) [e]

            THS.describe "parses" $ do
                let i = "--@cs :: {name: Text, age: Number, isFriendly: Bool}\n\
                    \SELECT * FROM Cats;"
                THS.it ("`" ++ i ++ "` into a correct AST") $ do
                    let t = Just [ TAST.TAstSimpleAtomicIndexKeyValue (TAST.TAstSimpleIndexKey "isFriendly") TAST.TAstAtomicTypeBool
                                 , TAST.TAstSimpleAtomicIndexKeyValue (TAST.TAstSimpleIndexKey "age") TAST.TAstAtomicTypeNumber
                                 , TAST.TAstSimpleAtomicIndexKeyValue (TAST.TAstSimpleIndexKey "name") TAST.TAstAtomicTypeText ]
                    let a = [AST.AstSelectAttributeAccessStar AST.AstSelectAttributeStarTotalRecord]
                    let f = [AST.AstFromAccessReference (AST.AstVariable "Cats")]
                    let e = AST.AstSelectQuery t a f
                    THS.shouldBe (p i) [e]
